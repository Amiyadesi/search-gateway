...
`quality` is `usable`, `thin`, or `non_text`. The classifier removes image
Markdown, raw URLs, and HTML before measuring readable characters, then checks
link density. It is domain-neutral: it does not contain compatibility rules for
individual websites. `thin` usually means too little prose or a navigation-link
pile and remains a degraded extraction; `non_text` means no meaningful readable
text remained and returns `success: false`. A screenshot can still be returned
...
- `status`: `complete | snippet | timeout | error`;
- `quality`: `usable | thin | non_text` for the content retained in that context;
- `elapsed_ms`: wall time for that source.

Low-quality extracted Markdown is never sent to the summary model. A usable
search snippet replaces it when available. The response sets `partial: true`
when any source is not complete and retains completed contexts instead of
turning one slow source into an empty failure.

The response also includes gateway-side wall-clock timings:
...
Timings do not include request queries, full URLs, headers, credentials, or
upstream response bodies. Non-complete contexts remain visible even when
`include_markdown` is false; only their Markdown body is hidden.
...
MCP transport timeouts are structured instead of returning a Python exception
string:
...
The outer SSH adapter uses `phase: ssh`. An HTTP `504` generated by the API is
not flattened; its structured gateway body is preserved in `error`.

### OAuth 2.1

The public MCP transports accept the legacy `MCP_ACCESS_TOKEN` and OAuth 2.1
authorization-code tokens. Discovery documents are exposed at:

- `/.well-known/oauth-protected-resource`
- `/.well-known/oauth-authorization-server`

The server supports public-client dynamic registration at `/oauth/register`,
PKCE `S256` authorization at `/oauth/authorize`, and code exchange at
`/oauth/token`. The `resource` value is echoed and bound to the signed access
token; supported resources are the gateway origin and the `/mcp/search` and
`/mcp/fns` endpoints. Authorization codes are short-lived and one-shot.
