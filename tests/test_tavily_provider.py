...
def test_tavily_provider_uses_configured_base_url(monkeypatch):
    provider = TavilyProvider(
        Settings(gateway_api_key="test", tavily_api_key="proxy", tavily_api_url="https://search.example/tavily/")
    )
    calls: list[str] = []
    ...
    assert calls == ["https://search.example/tavily/search"]
